...
export class HomeComponent implements OnInit {
  ...
  // Számértékek és objektumok a Dashboard-ra
  activeProductNumber: number = 0;  // Aktív termékek száma.
  activeCustomerNumber: number = 0; // Aktív vásárlók száma.
  unpaidOrderNumber: number = 0;    // Még nem fizetett rendelések száma.
  unpaidBillSum: number = 0;        // Még nem fizetett számlák összege.
  increasePercent: number = 0;      // Bevétel kommulált összegének növekedése százalékban kifejezve.
  incomeBillSum: number = 0;        // Kifizetett számlák összege (bevétel).
  newBillSum: number = 0;           // Új számlák összege (követelés).
  allBillSum: number = 0;           // Teljes számlázott összeg.
  newOrder_dataObj: any;            // Friss megrendelések: legutóbbi 5 megrendelés.
  ...
  ngOnInit(): void {
    ...
    this.listOrder$.subscribe((allOrderArray: Order[]) => {
      ...
      const orderNumberByStatusObj = this.getOrderNumberByStatusObj(allOrderArray);
      ...
      const [observedIntevallum, lastIncome, cumulativeSum, increasePercent] = this.getBalanceDataArray(allOrderArray, 10);
      ...
      this.newOrder_dataObj = this.getLatestOrderObj(allOrderArray, 5);
    });


    this.listBill$.subscribe((billArray: Bill[]) => {
      // Mérlegadatok megjelenítése
  ...
  // Rendelések státusz szerinti számát tartalmazó objektum
  private getOrderNumberByStatusObj(allOrderArray: Order[]): {} {
    ...
    return orderNumberByStatusObj;
  }


  // Az utolsó értékesítések bevétele, a bevétel kommulált összege és a százalékos növekedése
  private getBalanceDataArray(allOrderArray: Order[], observedRange: number): any[] {
    ...
    const increasePercent = ~~((cumulativeSum[observedRange - 1] - cumulativeSum[0]) / cumulativeSum[0] * 100);
    return [observedIntevallum, lastIncome, cumulativeSum, increasePercent];
  }
  ...
  // Mérlegadatok megjelenítéséhez a számlák objektuma
  ...
  // Termékek kategóriák szerinti számát tartalmazó objektum
  ...
  // Rendelések státusz szerinti megoszlása | kördiagram
  private showOrderPieChart(orderNumberByStatusObj: { [key: string]: number }) {
    const data = {
  ...
  // Rendelések száma státuszuk szerint | szalagdiagram
  private showOrderBarCart(orderNumberByStatusObj: { [key: string]: number }) {
    const data = {
      labels: Object.keys(orderNumberByStatusObj),
      series: [Object.values(orderNumberByStatusObj)]
    };
   ...
   // Termékek kategóriák szerinti aránya | kördiagram
  private showProductRangePieChart(productNumberByCategoryObj: { [key: string]: number }) {
    const data = {
  ...
  // Termékek kategóriák szerinti száma | szalagdiagram
  private showProductRangeBarCart(productNumberByCategoryObj: { [key: string]: number }) {
    const data = {
  ...
  // Az utolsó értékesítések bevétele | oszlopdiagram
  private showLastOrderIncome(dataX: any, dataY: any) {
    const data: any = {
      labels: dataX,
      series: [dataY]
    };
    ...
    new Chartist.Bar('#lastOrderIncome', data, optionslastOrderIncome, responsiveOptions);
  }

  // Bevétel kommulált összegének alkulása | vonaldiagram
  private showLastOrderCumulativeSum(dataX: any, dataY: any) {
    const data: any = {
      labels: dataX,
      series: [dataY]
    };
    ...
    new Chartist.Line('#lastOrderCumulativeSum', data, optionsOrderCumulativeSum);
  }



  // Aktív termékek száma:
  private getActiveProductNumber(product$: Observable<Product[]>): any {
    return product$.pipe(
  ...
  // Aktív vásárlók száma:
  private getActiveCustomerNumber(customer$: Observable<Customer[]>): any {
  ...
  // Még nem fizetett rendelések száma.
  private getUnpaidOrderNumber(order$: Observable<Order[]>): any {
    return order$.pipe(
  ...
  // Még nem fizetett számlák összege.
  private getUnpaidBillSum(bill$: Observable<Bill[]>): any {
    return bill$.pipe(
