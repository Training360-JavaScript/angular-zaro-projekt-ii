...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getActiveProductNumber(this.listProduct$).subscribe(
      (active: number) => (this.activeProductNumber = active)
    );

    // Aktív vásárlók száma:
    this.getActiveCustomerNumber(this.listCustomer$).subscribe(
      (active: number) => {
    ...
    this.getOrderNumberByStatus(this.listOrder$).subscribe(
      (orderNumberByStatus: number[]) => {
    ...
    this.getProductNumberByCategory(this.listProduct$).subscribe(
      (productNumberByCategory: number[]) => {
    ...
    this.listOrder$.subscribe((allOrderArray) => {
      ...
      let lastIncome = allOrderArray.map(
        (item) => item.amount * Number(item.product?.price)
      );
      let cumulativeSum = [0];
      lastIncome.forEach(
        (val, i) =>
          (cumulativeSum[i] = Number(val) + cumulativeSum[i - 1] || Number(val))
      );
      ...
      this.increasePercent = ~~(
        ((cumulativeSum[9] - cumulativeSum[0]) / cumulativeSum[0]) *
        100
      );
  ...
  showLastOrderIncome(dataX?: any, dataY?: any) {
    ...
    const lastOrderIncome = new Chartist.Bar(
      '#lastOrderIncome',
      data,
      optionslastOrderIncome,
      responsiveOptions
    );
    lastOrderIncome;
  ...
  showLastOrderCumulativeSum(dataX?: any, dataY?: any) {
    ...
    const lastOrderCumulativeSum = new Chartist.Line(
      '#lastOrderCumulativeSum',
      data,
      optionsOrderCumulativeSum
    );
  ...
  showProductRangePieChart(orderData: number[]) {
    ...
    const options = {
      labelInterpolationFnc: function (value: any, idx: any) {
        return (
          Math.round(
            (data.series[idx] / data.series.reduce((a, b) => a + b)) * 100
          ) + '%'
        );
      },
    };
  ...
  showProductRangeBarCart(orderData: number[]) {
    const data = {
      ...
      series: [
        [orderData[0], orderData[1], orderData[2], orderData[3], orderData[4]],
      ],
    };
  ...
  _getOrderNumberByStatus(orderByStatus: any): any {
    return [
      orderByStatus.new.amountSum,
      orderByStatus.paid.amountSum,
      orderByStatus.shipped.amountSum,
    ];
  }
  ...
  getUnpaidBillSum(bill$: Observable<Bill[]>): any {
    return bill$.pipe(
      ...
      map(
        ([billAmount, orderAmount, productPrice]) =>
          billAmount * orderAmount * productPrice
      ),
